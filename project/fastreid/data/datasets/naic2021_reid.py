...
class NAIC2021ReidTrain(ImageDataset):
    ...
    def gen_sample_sets(self):
        # 'train_feature','reconstructed_query_feature/','extract_gallery_feature'
        train_dir = osp.join(self.dataset_dir, 'extract_gallery_feature_v3')
        query_dir = osp.join(self.dataset_dir, 'extract_gallery_feature_v3')
        gallery_dir = osp.join(self.dataset_dir, 'extract_gallery_feature_v3')
        required_files = [
            train_dir,
            query_dir,
            gallery_dir,
        ...
        train_set = lambda: self.process_dir(
            train_dir, osp.join(self.dataset_dir, 'train_list_stage2.txt'), 0
        )
...
@DATASET_REGISTRY.register()
class TestByte64(NAIC2021ReidTrain):
    _junk_pids = [-1]
    dataset_name = "TestByte64"

    def gen_sample_sets(self):
        query_dir = osp.join('reconstructed_query_feature/64')
        gallery_dir = osp.join('extract_feature')
        # gallery_dir = osp.join('gallery_feature')  ####################
        required_files = [
            query_dir,
            gallery_dir,
        ]
...
@DATASET_REGISTRY.register()
class TestByte128(NAIC2021ReidTrain):
    _junk_pids = [-1]
    dataset_name = "TestByte128"

    def gen_sample_sets(self):
        query_dir = osp.join('reconstructed_query_feature/128')
        gallery_dir = osp.join('extract_feature')
        # gallery_dir = osp.join('gallery_feature')  ####################
        required_files = [
            query_dir,
...
@DATASET_REGISTRY.register()
class TestByte256(NAIC2021ReidTrain):
    _junk_pids = [-1]
    dataset_name = "TestByte256"

    def gen_sample_sets(self):
        query_dir = osp.join('reconstructed_query_feature/256')
        gallery_dir = osp.join('extract_feature')
        # gallery_dir = osp.join('gallery_feature')  ####################
        required_files = [
            query_dir,
